...
url = "https://uos.ac.kr/korNotice/list.do?list_id=FA1&epTicket=LOG#"
get_url = get(url)
...
for exam in range(len(title_result)-10, len(title_result)):
    title_result[exam] = title_result[exam][5:]
...
for list in final_result:
    listStr += '<li class="list-group-item"><a href="/proj/test_desc_norm.py?id={addr}">{title}</a></li>'.format(addr = list[0], title = list[1])
