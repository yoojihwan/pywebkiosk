...
url = "https://uos.ac.kr/korNotice/list.do?list_id=FA1&seq=0&sort=&pageIndex="+str(urlIndex)+"&searchCnd=&searchWrd=&cate_id=&viewAuth=Y&writeAuth=Y&board_list_num=10&lpageCount=10&epTicket=LOG"
get_url = get(url)
...
for list in final_result:
    listStr += '<li class="list-group-item"><a href="/proj/desc_norm.py?id={addr}">{title}</a></li>'.format(addr = list[0], title = list[1])
